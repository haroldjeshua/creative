      ...
      <nav className="mx-auto flex max-w-screen-md gap-2">
        {routes.map((route: RouteItem) => (
          <Link
            ...
            key={route.href}
            className={cn(
              "relative flex min-w-16 flex-col gap-4 rounded-lg bg-background p-2 transition-colors ease-linear sm:min-w-24 sm:gap-8 sm:p-3",
          ...
          >
            ...
            {isTabActive(route.href) && (
              <span
                className={cn(
                  "absolute bottom-1 h-[2px] w-2 rounded-sm bg-background sm:bottom-2 sm:h-1 sm:w-4",
                )}
              />
            )}
          </Link>
